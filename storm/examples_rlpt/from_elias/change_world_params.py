...
def modify_dict(world_yml):
    ...
    indexes_spheres = select_random_indexes(0, 10, 10)
    indexes_cubes = select_random_indexes(11, 37, 10)
...
def modify_objects(world_model, n):
    changed_objects = []
    changed_dictionary = {}
    modified_objects = {}

    for obj_type, obj_data in world_model['coll_objs'].items():
        for obj_name, obj_info in obj_data.items():
            if obj_type == 'sphere' and random.random() <= 0.5:  # Randomly select spheres
                changed_objects.append((obj_type, obj_name))
                new_position = generate_random_position(n)
                obj_info['position'] = new_position
                changed_dictionary[obj_name] = obj_info
            elif obj_type == 'cube' and random.random() <= 0.5:  # Randomly select cubes
                changed_objects.append((obj_type, obj_name))
                new_position = generate_random_quaternion()
                obj_info['pose'][3:] = new_position
                changed_dictionary[obj_name] = obj_info
            modified_objects[obj_type] = changed_dictionary

    return changed_objects, world_model, modified_objects
